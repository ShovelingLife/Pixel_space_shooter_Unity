...
{
    public Dictionary<KeyCode, Action> single_key_fn_dic =      new Dictionary<KeyCode, Action>();
    public Dictionary<KeyCode, Action> continuous_key_fn_dic =  new Dictionary<KeyCode, Action>();
    public Dictionary<KeyCode, Action> single_key_up_fn_dic =   new Dictionary<KeyCode, Action>();
}

[Serializable]
public class Player_power_up_info
{
    // 플레이어 보호막 파워업 관련
...
public class Player_manager : Singleton_local<Player_manager>
{
    ...
    public Player_power_up_info player_power_up_info;
    ...
    GameObject   player_death_anim_obj;
    bool         is_player_dead;
    ...
    void Update()
    {
        ...
        if (!is_player_dead )
        {
            ...
            // Power up
            Create_shield();
            Create_missile(player_power_up_info.missile_level);
    ...
    {
        if (player_power_up_info.is_shield_created)
        {
    ...
    void Init_player_settings()
    {
        ...
        player_death_anim_obj           = this.gameObject.transform.GetChild(1).gameObject;
        player_power_up_info.shield_obj = this.gameObject.transform.GetChild(2).gameObject;
    ...
    void First_time()
    {
        if (m_is_first_time)
        {
            ...
            if (m_current_time_count > m_timer)
            {
                ...
                UI_manager.instance.Run_start_text_obj();
            }
        }
    }
    ...
    {
        // 키를 한 번 눌렀을 시
        player_key.single_key_fn_dic[KeyCode.P]              = Show_pause_menu; // P키 (정지/재개)

        // 키를 연속적으로 눌렀을 시
        player_key.continuous_key_fn_dic[KeyCode.UpArrow]    = Player_moving_up;    // 윗키
        player_key.continuous_key_fn_dic[KeyCode.LeftArrow]  = Player_moving_left;  // 왼쪽키
        player_key.continuous_key_fn_dic[KeyCode.DownArrow]  = Player_moving_down;  // 아랫키
        player_key.continuous_key_fn_dic[KeyCode.RightArrow] = Player_moving_right; // 오른쪽키
        player_key.continuous_key_fn_dic[KeyCode.Space]      = Player_shoot_bullet; // 발사키

        // 키를 눌렀다 땠을 시
        player_key.single_key_up_fn_dic[KeyCode.UpArrow]     = Player_stopped_moving; // 윗키
        player_key.single_key_up_fn_dic[KeyCode.LeftArrow]   = Player_stopped_moving; // 왼쪽키
        player_key.single_key_up_fn_dic[KeyCode.DownArrow]   = Player_stopped_moving; // 아랫키
        player_key.single_key_up_fn_dic[KeyCode.RightArrow]  = Player_stopped_moving; // 오른쪽키
    }
    ...
    {
        foreach (var input_key in player_key.single_key_fn_dic)
        {
    ...
    void Player_input_continuous_key()
    {
        ...
        foreach (var input_key in player_key.continuous_key_fn_dic)
        {
    ...
    {
        foreach (var key_input in player_key.single_key_up_fn_dic)
        {
    ...
    {
        float bullet_reload_time = Activate_player_bullet_booster(player_power_up_info.is_booster_on);

        if (player_bullet_data.bullet_timer > bullet_reload_time)
        {
            Activate_player_power_up(player_power_up_info.power_up_level);
    ...
    {
        if (player_power_up_info.is_shield_created)
            player_power_up_info.shield_obj.SetActive(true);

        else
            player_power_up_info.shield_obj.SetActive(false);
    }
    ...
    void Create_missile(int _missile_level)
    {
        ...
        player_power_up_info.missile_current_time += Time.deltaTime;

        if (player_power_up_info.missile_current_time > player_power_up_info.missile_reload_time)
        {
            ...
            for (int i = 0; i < _missile_level; i++)
            {
                ...
                if (missile_obj_arr[i] != null)
                {
                    if (i == 0) // 왼쪽 방향
                    {
                        missile_obj_arr[i].transform.position = player_power_up_info.missile_first_level_trans.position;
                        player_missile.current_direction      = "Left_direction";
                    }
                    if (i == 1) // 오른쪽 방향    
                    {
                        missile_obj_arr[i].transform.position = player_power_up_info.missile_second_level_trans.position;
                        player_missile.current_direction      = "Right_direction";
                    }
                    missile_obj_arr[i].SetActive(true);
                }
            }
            player_power_up_info.missile_current_time = 0f;
        }
    }
    ...
    float Activate_player_bullet_booster(bool _is_booster_on)
    {
        ...
        if (_is_booster_on) 
            bullet_reload_time = player_bullet_data.bullet_reload_time / player_power_up_info.bullet_speed_up_data.increase_speed;
    ...
    void Activate_player_power_up(int _power_up_level)
    {
        ...
        for (int i = 0; i < _power_up_level+1; i++)
        {
            ...
            if (bullet_obj_arr[i] != null)
            {
                ...
                if (i == 1) 
                    bullet_obj_arr[i].transform.position = player_power_up_info.second_bullet_trans.position;

                if (i == 2) 
                    bullet_obj_arr[i].transform.position = player_power_up_info.third_bullet_trans.position;
    ...
    {
        player_death_anim_obj.SetActive(true);
        yield return new WaitForSeconds(1f);
        player_death_anim_obj.SetActive(false);        
        gameObject.GetComponent<SpriteRenderer>().color = Global.sprite_fade_color;
        is_player_dead = true;
        UI_manager.instance.Game_over();
    }
}